...
RKMK::usage = "RKMK[{\[Omega]1, \[Omega]2, \[Omega]3}, R0, {t, t0, t1, dt}]
	given a time dependent rotational velocity {\[Omega]1, \[Omega]2, \[Omega]3} and the initial value R0 of
	the rotation matrix to be integrated, numerically integrates the R differential equation
	using a RK Munthe-Kaas scheme.
	The integration proceeds in steps of dt from t0 to t1.";
...
(* Runge Kutta Munthe-Kaas Method *)
RKMKStep[\[Omega]_List,w_,R0_, {t_,t0_,dt_}] :=
	Module[{w0,f,wout,Rout},
		w0={$MachineEpsilon,$MachineEpsilon,$MachineEpsilon};
		f=dexpInv[w].\[Omega];
		wout=RKStep[Append[f,1],Append[w,t],Append[w0,t0],dt];
		wout=Drop[wout,-1];
		Rout=MatrixExp[Hat[wout]].R0
	];
RKMK[\[Omega]_List, R0_, { t_,t0_,t1_, dt_}] :=
	Module[{i,w,w1,w2,w3,Rint, tint, Rlist},
		w={w1,w2,w3};
		tint=t0;
		Rint = R0;
		Rlist={Rint};
		For[i=1, i<=Round[(t1-t0)/dt],i++,
			Rint=RKMKStep[ \[Omega],w, Rint, {t, tint, dt} ];
			tint = tint + dt;
			AppendTo[Rlist,Rint];
		];
		Rlist
